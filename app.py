...
# --- eski "google.generativeai" SDK'sı yerine güncel "google-genai" SDK'sı ---
...
# --- Google Search grounding tool'u ---
...
def chat():
    ...
    # Gemini key varsa önce Gemini denenir; limit/hata durumunda Groq'a düşülür.
    use_gemini = bool(GEMINI_KEY) and bool(gemini_client)

    def stream_groq():
        """Groq ile stream cevap üretir. Üretilen tam metni generator'ın return değeri olarak verir."""
        reply_parts = []
        ...
        if use_gemini:
            ...
            for m_name in candidate_models:
                try:
                    ...
                    # diğer modelleri denemeye devam et.
                    continue

            if not success:
                gemini_failed = True
                logging.warning(f"Gemini tüm modellerde başarısız oldu, Groq'a geçiliyor. Son hata: {last_err}")

        # Gemini hiç kullanılmadıysa (key yok) veya Gemini başarısız olduysa -> Groq'a düş
